...
volatile float current_angle = 90.0f;
#define RX_BUF_SIZE 32
uint8_t rx_byte;
char rx_buffer[RX_BUF_SIZE];
uint8_t rx_index = 0;

#define ANGLE_DEADBAND 2.0f   // stopnie
volatile float bh1750_lux_live = 0.0f;
volatile float target_angle_live = 0.0f;
volatile float current_angle_live = 0.0f;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	char uartBuf[64]; // Bufor na tekst
		int len;
  /* USER CODE END 1 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  uint8_t buttonState = HAL_GPIO_ReadPin(USER_Btn_GPIO_Port, USER_Btn_Pin);
